# GPU VM build log — Intel Arc Pro B70 passed whole to one LLM guest

> 📦 **Archived 2026-09-16 — completed build, kept as the troubleshooting
> record.** This is the phase-by-phase log of bringing up VM 105 between
> 2026-09-15 and 2026-09-16: storage, PCI passthrough, the ATS hard-lockup and
> its `pci=noats` fix, the ReBAR resize saga, and the first llama.cpp build and
> benchmarks. For what the VM actually runs today — services, ports, presets,
> how to reach it — see [`../GPU-VM.md`](../GPU-VM.md). Read this file when a
> similar passthrough problem recurs and the exact sequence that fixed it
> matters (e.g. the `15 (ENOSPC) → 12 → 15` BAR-resize order, or the
> `--purge`-deletes-ACLs trap).
