...
**Reloader** `v1.4.22` — restarts a workload when a Secret or ConfigMap it
...
**cert-manager** `v1.21.2` — present **only** because the barman plugin hard-requires
...
.github/workflows/        renovate.yaml (dependency PRs), validate-manifests.yaml (kustomize build, helm template,
                          dependency-graph check), doc-history.yaml (warn-only docs check)
.github/scripts/          render-charts.py (renders every HelmRelease), depgraph.py (the dependsOn graph below),
...
<!-- depgraph:start -->
<!-- Generated by .github/scripts/depgraph.py --write README.md; edit the ks.yaml files, not this block. -->
```mermaid
flowchart LR
  subgraph ns_cert_manager["cert-manager"]
    cert_manager["cert-manager"]
  end
  subgraph ns_cnpg_system["cnpg-system"]
    cloudnative_pg["cloudnative-pg"]
    plugin_barman_cloud["plugin-barman-cloud"]
  end
  subgraph ns_infisical["infisical"]
    infisical_secrets_operator["infisical-secrets-operator"]
  end
  ...
  subgraph ns_sunfire["sunfire"]
    ...
    sunfire_infisical["sunfire-infisical"]
    sunfire_minio["sunfire-minio"]
    sunfire_offsite_backup["sunfire-offsite-backup"]
    sunfire_postgres["sunfire-postgres"]
  ...
  sunfire_minio --> sunfire_cloudflared
  sunfire_postgrest --> sunfire_cloudflared
  infisical_secrets_operator --> sunfire_infisical
  sunfire_storage --> sunfire_minio
  sunfire_minio --> sunfire_offsite_backup
  sunfire_postgres_cnpg --> sunfire_offsite_backup
  sunfire_storage --> sunfire_postgres
  plugin_barman_cloud --> sunfire_postgres_cnpg
  sunfire_minio --> sunfire_postgres_cnpg
  sunfire_postgres --> sunfire_postgres_cnpg
  sunfire_postgres_cnpg --> sunfire_postgrest
  cloudnative_pg --> voice_db
```
<!-- depgraph:end -->
...
> The `sunfire-postgres` node is not a database. It holds only the
> `postgres-credentials` Secret, which `sunfire-postgres-cnpg` reads for its
> `externalClusters` reference. PostgREST depends on `sunfire-postgres-cnpg`,
> the database it actually connects to.
