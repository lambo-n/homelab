...
- [`homelab/`](homelab/): **GitOps source of truth for the k3s cluster** (`https://github.com/lambo-n/homelab`, private, branch `main`). Flux + SOPS. Contains `kubernetes/apps/sunfire/{minio,postgres,postgrest,cloudflared,storage}` as `ks.yaml` + `app/`, with every secret encrypted as `*.sops.yaml`. Toolchain pinned in `mise.toml`. See its `README.md` and [`GITOPS.md`](homelab/GITOPS.md).
...
- [`sunfire-backend/`](sunfire-backend/): Live Kubernetes manifests (`deployment.yaml`, `service.yaml`, `pv.yaml`, `pvc.yaml`), role bootstrap script (`bootstrap-roles.sql`), and plaintext live secrets (`chmod 600`), including [`sunfire-backend/minio/worker-credentials.yaml`](sunfire-backend/minio/worker-credentials.yaml) which stores the scoped MinIO service account keys for Cloudflare Workers. Relocated from `~/sunfire/` when the app repo was cloned to keep the git repository clean and prevent unencrypted secrets from being committed.
- `bingobackups/` & [`BINGO.md`](BINGO.md): Archived, decommissioned assets from the retired Sun Clan Bingo app (read-only historical reference).
- [`homelab/GITOPS.md`](homelab/GITOPS.md): Planning notes and execution checklist for migrating the homelab infrastructure to GitOps (Flux + SOPS). Moved into the repo 2026-09-02 (was `~/GITOPS.md`).
- `pool-rebuild.sh`, `reboot-worker*.sh`: Cluster and VM operational scripts.
...
- `POSTGREST_JWT_SECRET`: Staged in both `production` and `feature` environments (byte-matched to [`sunfire-backend/postgrest/secret.yaml`](sunfire-backend/postgrest/secret.yaml)).
- `MINIO_ACCESS_KEY` & `MINIO_SECRET_KEY`: Staged in both environments from the scoped service accounts in [`sunfire-backend/minio/worker-credentials.yaml`](sunfire-backend/minio/worker-credentials.yaml).
- `CF_ACCESS_CLIENT_ID` & `CF_ACCESS_CLIENT_SECRET`: Pending generation in the Cloudflare Zero Trust dashboard at tunnel cutover. **One `sunfire-worker` service token is shared by the production Worker, the feature Worker and local dev** — same value in all three, deliberately (see `GITOPS.md` → "Explicitly rejected"). This is *not* an oversight to correct, even though MinIO one layer below uses two service accounts for independent revocation; the asymmetry is intentional. The tunnel token is separate again and is cluster-only — it lives in the `cloudflared-token` Secret and no Worker or local dev ever sees it.
...
3. **Cluster Manifests:** [`homelab/`](homelab/) is the source of truth for cluster manifests as of 2026-09-02. [`sunfire-backend/`](sunfire-backend/) holds the pre-GitOps hand-applied copies and the **plaintext** secrets; it is the rollback path and the origin of the encrypted secrets, not a place to make new changes. Note that as of Phase 2 the cluster is still running the hand-applied manifests — **Flux is not installed yet**, so a change in `homelab/` does not reach the cluster on its own.
4. **Never nest the repos.** `/home/dev` is deliberately *not* a git repository. `homelab/`, `sunfire/`, and `sunfire-backend/` are independent siblings; do not `git init` in `/home/dev` or move one inside another.
