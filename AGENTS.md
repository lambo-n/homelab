...
3. **Cluster Manifests:** [`homelab/`](homelab/) is the source of truth for cluster manifests as of 2026-09-02. [`sunfire-backend/`](sunfire-backend/) holds the pre-GitOps hand-applied copies and the **plaintext** secrets; it is the rollback path and the origin of the encrypted secrets, not a place to make new changes. **Flux reconciles this repo into the cluster**, so a change committed and pushed to `main` reaches the cluster on its own — and `prune: true` means removing a manifest from git deletes the live object.
